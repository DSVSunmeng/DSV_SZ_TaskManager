...
│                                                                                  │
│  ┌─────────────────┐   ┌──────────────────┐   ┌──────────────────────────────┐  │
│  │ WS 线程          │   │ 消息队列 Queue    │   │ ThreadPoolExecutor(2)        │  │
│  │ 纯接收 + ACK     │ → │ FIFO + 5min TTL  │ → │ 单条消息 150s 超时保护       │  │
│  │ @提及预处理       │   │ 去耦 WS 与业务    │   │ _process_one_message()      │  │
│  └─────────────────┘   └──────────────────┘   └──────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────────────────────┘
                                                   │
                          ┌────────────────────────┼────────────────────┐
                          ...
                          ▼                        ▼                    ▼
              ┌────────────────────┐  ┌────────────────────┐  ┌──────────────────┐
              │ project_matcher.py │  │  妙搭 OpenAPI      │  │ on_task_created  │
              │ 项目缩写匹配        │  │  AI 任务拆解        │  │ 回调分发          │
...
- 4-tuple (text, chat_id, open_id, timestamp): 统一格式避免解包错误
```
...
替换: "你好 张三(@ou_xxx) 请处理"
               ↓
妙搭收到: 可读姓名；handler 收到: 可解析 open_id
```

---

### 3.2 miaoda_task_handler.py — 任务创建核心
...
Step 7: 回调处理
  on_task_created():
  · bitable_writer.write_task_to_bitable()  ← 非阻塞
  · notify_assignee.notify_assignee()       ← 非阻塞
...
| 多维表格写入失败 | 记录日志 warning，不阻止主流程 | 回复包含创建成功信息 |
| 通知发送失败 | 记录日志 warning，不阻止主流程 | 同上 |
